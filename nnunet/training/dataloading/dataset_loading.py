...
class SS_DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch_semi(self):
        ...
        selected_keys_labeled = np.random.choice(self.list_of_keys, label_count, True, None)
        selected_keys_pseudo = np.random.choice(self.list_of_keys_pseudo, pseudo_label_count, True, None)
        selected_keys = np.concatenate((selected_keys_labeled, selected_keys_pseudo))
        ...
        for j, i in enumerate(selected_keys):
            ...
            if j < label_count:
                # labeled data
                force_fg = True
                _data_dict = self._data
            else:
                # pseudo-labeled data
                force_fg = False
                _data_dict = self.pseudo_data

            if 'properties' in _data_dict[i].keys():
                properties = _data_dict[i]['properties']
            else:
                properties = load_pickle(_data_dict[i]['properties_file'])
            case_properties.append(properties)

            # cases are stored as npz, but we require unpack_dataset to be run. This will decompress them into npy
            # which is much faster to access
            if isfile(_data_dict[i]['data_file'][:-4] + ".npy"):
                case_all_data = np.load(_data_dict[i]['data_file'][:-4] + ".npy", self.memmap_mode)
            else:
                case_all_data = np.load(_data_dict[i]['data_file'])['data']
...
class SS_DataLoader2D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
                 memmap_mode="r", pseudo_3d_slices=1, pad_mode="edge",
                 pad_kwargs_data=None, pad_sides=None, pseudo_data=None):
        """
        This is the basic data loader for 2D networks. It uses preprocessed data as produced by my (Fabian) preprocessing.
        You can load the data with load_dataset(folder) where folder is the folder where the npz files are located. If there
        are only npz files present in that folder, the data loader will unpack them on the fly. This may take a while
    ...
    def generate_train_batch_semi(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not isfile(_data_dict[i]['data_file'][:-4] + ".npy"):
                # lets hope you know what you're doing
                case_all_data = np.load(_data_dict[i]['data_file'][:-4] + ".npz")['data']
            else:
                case_all_data = np.load(_data_dict[i]['data_file'][:-4] + ".npy", self.memmap_mode)
