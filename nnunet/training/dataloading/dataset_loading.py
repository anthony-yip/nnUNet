...
class SS_DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None, pseudo_data=None):
        # data is a dictionary of patient identifiers, each pointing to another dictionary with the data filepath,
        # properties filepath and properties dictionary
        # pseudo_data is similar, but only containing data and no properties
        """
        This is the basic data loader for 3D networks. It uses preprocessed data as produced by my (Fabian) preprocessing.
        You can load the data with load_dataset(folder) where folder is the folder where the npz files are located. If there
        are only npz files present in that folder, the data loader will unpack them on the fly. This may take a while
    ...
    def generate_train_batch(self):
        if self.pseudo_data is None:
            ...
            assert self.patch_size == self.final_patch_size, "since pseudo labels are only self.final_patch_size in size"
            self.generate_train_batch_semi()
        else:
            raise RuntimeError

    def generate_train_batch_full(self):
        selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)
        data = np.zeros(self.data_shape, dtype=np.float32)
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        case_properties = []
    ...
    def generate_train_batch_semi(self):
        label_count = max(round(self.batch_size * self.pseudo_percentage), 1)
        pseudo_label_count = self.batch_size - label_count

        selected_keys = np.random.choice(self.list_of_keys, label_count, True, None)
        selected_keys_pseudo = np.random.choice(self.list_of_keys_pseudo, pseudo_label_count, True, None)
        data = np.zeros(self.data_shape, dtype=np.float32)
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        case_properties = []
        for j, i in enumerate(selected_keys):
            ...
            for d in range(3):
                assert case_all_data.shape[d + 1] > self.patch_size[d], "regular labeled data is smaller than the patch size"
            ...
            # We need to make sure we get at least one of the foreground classes in the patch
            # these values should have been precomputed
            if 'class_locations' not in properties.keys():
                raise RuntimeError("Please rerun the preprocessing with the newest version of nnU-Net!")
            ...
            data[j] = np.copy(case_all_data[:-1, bbox_x_lb:bbox_x_ub, bbox_y_lb:bbox_y_ub, bbox_z_lb:bbox_z_ub])
            seg[j, 0] = np.copy(case_all_data[-1:, bbox_x_lb:bbox_x_ub, bbox_y_lb:bbox_y_ub, bbox_z_lb:bbox_z_ub])

        for a, b in enumerate(selected_keys_pseudo):
            if 'properties' in self.pseudo_data[b].keys():
                properties = self.pseudo_data[b]['properties']
            else:
                properties = load_pickle(self.pseudo_data[b]['properties_file'])
            case_properties.append(properties)

            # cases are stored as npz, but we require unpack_dataset to be run. This will decompress them into npy
            # which is much faster to access
            if isfile(self.pseudo_data[b]['data_file'][:-4] + ".npy"):
                case_all_data = np.load(self.pseudo_data[b]['data_file'][:-4] + ".npy", self.memmap_mode)
            else:
                case_all_data = np.load(self.pseudo_data[b]['data_file'])['data']
        ...
        keys = np.concatenate((selected_keys, selected_keys_pseudo))
        assert len(data) == len(seg) == len(case_properties) == len(keys)
